...
namespace evo::gui
{
	...
	void about_popup::render()
	{
		...
		// ev0lve Digital
		const auto ev0_size = fnt_bold->get_text_size(XOR("ev0lve"));
		...
		d->add_text(txt_pos, XOR("ev0lve"), colors.accent);
		d->add_text(txt_pos + vec2{ ev0_size.x, 0.f }, XOR(".us"), colors.text);
		d->font = fnt;
		d->add_text(txt_pos + vec2{ 0.f, 16.f }, tfm::format(XOR(" 2023-%i."), tm.tm_year + 1900),
			colors.text_mid);
