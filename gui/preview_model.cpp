...
void preview_model::render()
{
	...
	//d->g.texture = ent_ren.render_model;
	//
	//d->add_rect_filled(evo::ren::rect(padding, { (float)entity_rendering::preview_texture->GetActualWidth(), (float)entity_rendering::preview_texture->GetActualHeight() }), color(255, 255, 255));
	//d->g.texture = {};
	...
	{
		d->font = draw.fonts[FNV1A("tahoma13")];
	...
	d->font = draw.fonts[FNV1A("tahoma8")];
	...
	{
		const auto tex =
			draw_mgr.adapter.icon_to_texture(XOR_STR("materials/panorama/images/icons/equipment/zeus.svg"), 12.f);
		if (tex.has_value() && tex->texture)
		{
			draw_icon(tex, { bb.mins.x - padding.x - 7, bb.mins.y + pad }, 12.f, color::white());
			pad += 12;
	...
	if (esp->get().test(cfg_t::esp_flags))
	{
		...
		pad += 9;
		d->add_text({bb.maxs.x + padding.z, bb.mins.y + pad}, XOR_STR("hk"), col_to_col(sdk::color::white()));
		pad += 9;
		d->add_text({bb.maxs.x + padding.z, bb.mins.y + pad}, XOR_STR("kit"), col_to_col(sdk::color::info()));
		pad += 9;
	}
