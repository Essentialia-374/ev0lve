...
class cs_player_t : public entity_t
{
	...
	inline void set_icon(int level) {
		if (!game->cs_player_resource)
			return;

		/*game->cs_player_resource->m_nPersonaDataPublicLevel()[index()] = level;*/
		printf("[ev0lve] not implemented | %i | %i", level, index());
	}
};
