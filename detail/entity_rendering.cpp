...
void entity_rendering::draw()
{
	...
	fake_player_model->root.mdl.time += game->globals->frametime * 0.5f;
	...
	game->render_view->GetMatricesForView(view_setup, &v1, &v2, &world_to_projection, &v3);
	...
	ctx->SetRenderTarget(render_target);
	ctx->BindLocalCubemap(cubemap);
	ctx->SetLightingOrigin({0.f, 0.f, 100.f});
	ctx->SetIntRenderingParameter(10, 0);

	sdk::frustum_t frustum;
	game->render_view->Push3DView(ctx, view_setup,
	...
	game->render_view->PopView(ctx, frustum);
...
void entity_rendering::handle_fake_player()
{
	...
	fake_player_model->set_sequence(113, false);
