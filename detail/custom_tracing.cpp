...
namespace detail
{
		...
		cs_weapon_data* override_data, int32_t expected_hitgroup)
	{
		...
		const auto wpn = override_data ? nullptr :
			reinterpret_cast<weapon_t*>(game->client_entity_list->get_client_entity_from_handle(
		...
		if (target) {
			// Create filter on stack - it handles its own vtable
			trace_friendly_filter filter(player);
			result = fire_bullet(
				wpn->get_cs_weapon_data(),
				src,
				end,
				&filter,
				target,
			...
			if (override_data) {
				...
				// Create filter on stack with proper vtable initialization
