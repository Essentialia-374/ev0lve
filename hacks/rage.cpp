...
void rage::autostop(sdk::user_cmd* const cmd, const bool should_stop, const bool force, bool full)
{
	...
	const auto should_slide = (cfg.rage.slowwalk.get() && (cfg.rage.slowwalk_mode.get().test(cfg_t::slowwalk_speed)
		|| aa.determine_maximum_lag() < 8
		|| game->client_state->choked_commands < aa.determine_maximum_lag() - 3)) || !cfg.rage.slowwalk.get();

	// calculate new speed.
	const auto slide_speed = (wpn->get_item_definition_index() == item_definition_index::weapon_revolver ? 180.f : wpn->get_max_speed()) / 4.f - 1.f;
