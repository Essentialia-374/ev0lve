...
void on_post_data()
{
	...
	if (player_map->data_desc != player_types.data())
	{
		...
		player_types.emplace_back(
			field_boolean, reinterpret_cast<recv_prop*>(game->client.at(0xbfeff8))->name,
			0x99bc, 1);
		player_types.emplace_back(field_integer,
			reinterpret_cast<recv_prop*>(game->client.at(0xbff024))->name,
			0x99d0, 4);
		player_types.emplace_back(field_boolean,
			reinterpret_cast<recv_prop*>(game->client.at(0xbffe5c))->name,
			0x9ae0, 1);
		player_types.emplace_back(
			field_float,
			reinterpret_cast<recv_prop*>(game->client.at(0xbfed48))->name,
			0x103ec, 4, game->globals->interval_per_tick / 2.5f);
		player_types.emplace_back(
			field_float, reinterpret_cast<recv_prop*>(game->client.at(0xbffe44))->name,
			0x119ec, 4, .01f);
	...
	if (grenade_map->data_desc != grenade_types.data())
	{
		...
		grenade_types.emplace_back(
			field_boolean,
			reinterpret_cast<recv_prop*>(game->client.at(0xc83818))->name,
			0x33e1, 1);
		grenade_types.emplace_back(
			field_float, reinterpret_cast<recv_prop*>(game->client.at(0xc83844))->name,
			0x33e4, 4, .01f);

		grenade_map->data_desc = grenade_types.data();
		grenade_map->data_num_fields = grenade_types.size();

		for (auto& el : std::array{ 0xdba138, 0xdba198, 0xdba300, 0xdba438, 0xdba480, 0xdba180 })
