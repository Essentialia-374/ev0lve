...
void grenade_prediction::draw()
{
	...
	/*	angle trajectory;
		...
		vec3 screen0, screen1;
		buf->g.anti_alias = true;
		if (world_to_screen(prev, screen0) && world_to_screen(cur, screen1))
			buf->add_line({ screen0.x, screen0.y }, { screen1.x, screen1.y }, { 60, 180, 225 });
...
;
	...
	ang_throw.x = pitch - (90.f - std::abs(pitch)) * 10.f / 90.f;
	...
	// weird formula, valve..
	vec_throw = game->local_player->get_velocity();
	vec_throw *= 1.25f;
	vec_throw += (forward * wpn->get_cs_weapon_data()->flthrowvelocity);
}
