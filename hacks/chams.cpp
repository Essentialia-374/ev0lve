						   ...
						   const std::function<void(mat3x4 *)> draw, const bool is_attachment)
{
		...
		game->cl_predict)
	{
		...
		if (second > 0.f)
		{
			...
			if (record && record->has_visual_matrix)
			{
				...
				if (dist > 0.f)
				{
					const auto backup_blend = game->render_view->GetBlend();
					game->render_view->SetBlend(dist);
					...
					game->model_render->forced_material_override(nullptr);
					game->render_view->SetBlend(backup_blend);
				}
			}
	...
	// TODO: this as well
	if (is_enemy && false)
		game->render_view->SetBlend(entry.visuals.dormant_alpha);
...
material *chams::maintain_material(cfg_t::chams &chams, int32_t index)
{
	...
	if (setup || base_color.rgba() != clr1[index])
	{
		...
		mat->find_var_fast(clr, &clr_id)->set_vec3(v);
		...
		(setup || secondary_color.rgba() != clr2[index]))
	{
		...
		mat->find_var_fast(tint, &tint_id)->set_vec3(v);
	}
