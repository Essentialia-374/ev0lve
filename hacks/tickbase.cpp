...
{
	const auto wpn = reinterpret_cast<weapon_t* const>(
		game->client_entity_list->get_client_entity_from_handle(
			game->local_player->get_active_weapon()));
	if (!wpn)
		return 0;

	const int limit = compute_current_limit();
	...
	// if the server‑side limit is smaller than our minimum, just use it 
	if (limit <= 4)
		return max(limit, 0); // never negative

	const int desired = TIME_TO_TICKS(wpn->get_cs_weapon_data()->cycle_time) - 1;
	return std::clamp(desired, 4, limit);
}
