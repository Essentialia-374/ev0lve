...
{
	if (!is_forced_output && (!cfg.misc.eventlog.get() || log.empty()))
	{
		log.clear();
		return;
	}
	...
	for (auto part = log_output.begin(); part != log_output.end(); ++part)
	{
		...
		// set string
		if (cfg.misc.eventlog.get() || is_forced_output)
		{
			// set current text
...
void event_log::on_player_hurt(sdk::game_event* event)
{
	...
	if (attacker_index == game->engine_client->get_local_player())
	{
		...
		add(0x09, XOR_STR("Hit "));
		add(0x09, victim_name);
		add(0x06, XOR_STR(" in the "));
		add(0x09, hitbox);
		add(0x06, XOR_STR(" for "));
		add(0x09, std::to_string(dmg));
		add(0x06, XOR_STR(" damage ("));
		add(0x09, std::to_string(health));
		add(0x06, XOR_STR(" health remaining)."));
	}
