...
void game_t::init()
{
	...
	while (!client_mode)
	{
		printf(XOR_STR("debug ~ waiting for client_mode...\n"));
	...
	dispatch.spawn();
	printf(XOR_STR("debug ~ dispatch spawn\n"));

	/* initialize some stuff in the cheat. */
	aim_helper::build_seed_table();
	printf(XOR_STR("debug ~ built seed table\n"));
	hacks::skin_changer->parse_game_items();
	printf(XOR_STR("debug ~ parsed game items\n"));
	hacks::skin_changer->parse_weapon_names();
	printf(XOR_STR("debug ~ parsed weapon names\n"));
	/* finish final init. */
	hooks::hl_client::level_init_pre_entity(nullptr, 0, nullptr);
	printf(XOR_STR("debug ~ level_init_pre_entity\n"));
	hacks::skin_changer->load_skins();
	printf(XOR_STR("debug ~ loaded skins\n"));
	ent_ren.init();
	printf(XOR_STR("debug ~ ent ren init\n"));
