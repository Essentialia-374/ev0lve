...
int get_vector(lua_State *l)
{
	...
	const auto vec = var->get_vec3();
	const auto vec_sz = var->get_vec3_size();
...
int set_vector(lua_State *l)
{
	...
	const auto var = s.user_data_ptr<sdk::material_var>(1);
	var->set_vec3_component(s.get_float(2), 0);
	var->set_vec3_component(s.get_float(3), 1);
	if (s.get_stack_top() >= 4)
		var->set_vec3_component(s.get_float(4), 2);
	if (s.get_stack_top() >= 5)
		var->set_vec3_component(s.get_float(5), 3);
